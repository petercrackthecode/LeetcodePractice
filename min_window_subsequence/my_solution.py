...
class Solution:
    def minWindow(self, s1: str, s2: str) -> str:
        ...
        - Once you’ve identified a potential end, backtrack_and_get_nearest_start through the characters of str1 from this end position until you’ve found all the characters of str2 in reverse order. This helps locate the potential start of the smallest subsequence.
        ...
        if p is greater than or equal to len(s2) => we have a valid window:
            - backtrack_and_get_nearest_start to get the minimum contiguous substring, then assign the result to ans if its length is smaller than ans. Have a helper function call backtrack_and_get_nearest_start(left, right) to help you do so. Then, reassign 0 to p.
        """
        p = 0
        left = 0
        ans = " " * (len(s1) + 1)

        def backtrack_and_get_nearest_start(left: int, right: int) -> int:
            nonlocal s1, s2, ans
            ...
            # i < left: no shorter valid substring within the range [left, right] (inclusive) in s1
        ...
        while right < len(s1) and p < len(s2):
            ...
            if p >= len(s2):
                ...
                nearest_start = backtrack_and_get_nearest_start(left, right)
                p = 0
                left = nearest_start + 1
                while left < len(s1) and s1[left] != s2[p]:
                    left += 1
                    right = left
                right = left
